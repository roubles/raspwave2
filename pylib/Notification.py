...
def fromDict(**entries):
    ...
    n = Notification()
    if entries["type"] is "ValueNotification":
        n = ValueNotification()
    if entries["type"] is "ValueChangeNotification":
        n = ValueChangeNotification()
    if entries["type"] is "BatteryValueNotification":
        n = BatteryValueNotification()
    if entries["type"] is "WakeupNotification":
        n = WakeupNotification()
    if entries["type"] is "NodeEventNotification":
        n = NodeEventNotification()
    n.__dict__.update(entries)
    return n

class Notification:
    birthMark = 0

    def __init__ (self, nodeId=0, commandClass=0, fullHex=0):
